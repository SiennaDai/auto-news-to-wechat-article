...
### AI 自动排版
- 三代理流水线（LangGraph）：Writer 生成结构化 HTML → ImgMatcher 语义匹配图片 → Checker 校验修复
...
```
图片预处理 → Writer → ImgMatcher → Checker → 文编处理 → 美编装饰 → Base64 嵌入
```

| 阶段 | 模块 | 说明 |
|------|------|------|
| 图片预处理 | `src/img_process.py` | 压缩大图（长边 1280px），生成图片清单 |
| AI 流水线 | `src/words_agents.py` | LangGraph 三代理：Writer → ImgMatcher → Checker |
...
```
├── app.py                    # Flask 服务入口（SSE 流式响应、REST API）
├── backend/                  # 用户系统
...
│   └── decoration_base.json  # 装饰模板库（6 类 × 各 6 种样式）
├── prompts/                  # AI 代理系统提示词
│   ├── writer.txt            # Writer 代理提示词
│   ├── imgmatcher.txt        # ImgMatcher 代理提示词
│   └── checker.txt           # Checker 代理提示词
└── data/                     # 运行时数据
    ├── input/                # CLI 模式输入
